	...
	<div class="container">
		...
		<div class="col-sm-12 col-md-12">
				<div class="marginTop65px"></div>
					...
					<a onclick="return viewInfo('${toolName}')"><button class="btn btn-primary " id="help">Help</button></a> ${toolName} :: Runs 
						<a href='./newRun?toolName=${toolName}'><button type="button" class="btn btn-primary"
								id="addRun" title="Add New Run">
								Add new run <span class="glyphicon glyphicon-plus"></span>
								</button>
						</a> 
						
						<a href="getFramework?toolName=${toolName}"><button
							type="button" class="btn btn-primary pull-right" title="Download framework">
							Download<span class="glyphicon glyphicon-download-alt"></span>
							</button>
						</a>
					<a href="#">	<button type="button" onclick="commandExecute();" class="btn btn-primary pull-right" id="speak"><i class="fa fa-microphone" aria-hidden="true"></i></button></a>	
					<span id="speakResult"></span>
					</h1>
			</div>
		</div>
	</div>
			...
			<div class="col-sm-8 col-sm-offset-2">
				...
				<c:if test="${not empty successRun}">
					<div class="alert alert-success alert-dismissible" role="alert">
						...
						${successRun}<script>speak('${successRun }')</script>
					</div>
					<br>
				</c:if>
				...
				<c:if test="${not empty triggerMessage}">
					<div class="alert alert-success alert-dismissible" role="alert">
						...
						${triggerMessage}<script>speak('${triggerMessage }')</script>
					</div>
					<br>
				</c:if>
				...
				<c:if test="${not empty deleteMessage}">
					<div class="alert alert-success alert-dismissible" role="alert">
						...
						${deleteMessage}<script>speak('${deleteMessage }')</script>
					</div>
					<br>
				</c:if>
							...
							<c:if test="${ not empty runs}">
								...
								<table class="table table-striped table-bordered" id="runsTable">
									...
									<c:forEach var="run" items="${runs}">
										...
										<tr class="last">
											<td class="first"><a href="./editRun?runId=${run.runid}&toolName=${toolName}&runName=${run.runName}">${run.runName}</a></td>
