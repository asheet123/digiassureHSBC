...
public class DigitalQADaoImpl implements IDigitalQADao {
	...
	@Override
	public boolean documentTest(String baseDocument,String baseFilePath,String actualDocument, String actualFilePath, String resultFile,String userid) {
		String query = "INSERT INTO comparisionreport(basedocument,basepath,actualdocument,actualpath,reportpath,status,createdby,creationtime) VALUES (?,?,?,?,?,?,?,current_timestamp)";
		Object[] params = new Object[] {
				baseDocument,baseFilePath,actualDocument,actualFilePath,resultFile,"Y",userid
		};
		int count = 0;
		try {
			count = jdbcTemplate.update(query, params);
			...
			DigiLoggerUtils.log(
					"Error While Adding the documentTest :: Exception  :: "
							+ e.getMessage(), LEVEL.error);
		}
		if(count>0){
		return true;
	...
	public List<Comparisionreport> getDocumentTestReport(String projectid) {
		List<Comparisionreport> list = null;
		String query = "SELECT * FROM comparisionreport WHERE createdby IN (select userid from users where projectid = '"
				+ projectid + "' AND status = 'Y' ) AND status = 'Y'";
		try {
			list = jdbcTemplate.query(query,
					new BeanPropertyRowMapper<Comparisionreport>(Comparisionreport.class));
			DigiLoggerUtils.log(
					"Data while getDocumentTestReport", LEVEL.info);
		} catch (DataAccessException e) {
			DigiLoggerUtils.log(
					"Error while getting getDocumentTestReport :: Exception IS ::: " + e.getMessage(),
					LEVEL.error);
		}
		return list;
	}
	...
	public boolean deleteReport(String reportid) {
		int out = 0;
		try {
			out = jdbcTemplate.update("UPDATE comparisionreport SET status='N' WHERE comparisionreportid=" + reportid);
		} catch (DataAccessException e) {
			DigiLoggerUtils.log(
					"Error while delteing report :: report id " + reportid + ":Exception Message " + e.getStackTrace(),
					LEVEL.error);
		}
		if (out >= 1) {
